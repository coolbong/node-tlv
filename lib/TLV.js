 ...
 *
 * @param {string} [type] return type (number / string)
 * @returns {string|number} string value of the tag
 */
TLV.prototype.getTag =  function(type) {
    //string
    if (type === 'number') {
...
// //FIXME add html code make async api
// TLV.prototype.html = function (callback, depth) {
//     depth = depth || 0;
//     if(this.info.encoding == 'constructed') {
//         this.child.forEach(function(child){
//             child.html(callback, depth + 1);
//         });
//     } else {
//         callback(this.tag, this.length, this.value, false, depth);
//     }
// };
...
DOL.prototype.setValue = function(values) {
    ...
    var offset = 0;
    this.list.forEach(function(item) {
        item.setValue(values.slice(offset, offset + item.getLength()));
