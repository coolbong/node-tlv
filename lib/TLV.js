...
function TLV (tag, value, encoding) {
    ...
    if (Buffer.isBuffer(this.value)) {
        this.bValue = this.value;
    } else if (typeof this.value == 'string') {
        this.bValue = new Buffer(this.value, 'hex');
    } else {
        this.bValue = new Buffer();
    }

    // buffered data
    this.bTag = new Buffer(this.tag, 'hex');
    this.bLength = TLV.getBufferLength(this.value, this.encodingMode);
 ...
 * @returns {Buffer}
 */
TLV.getBufferLength = function(data, encoding) {
...
function main() {
    var acKey = new TLV('', 'FE5960267173B426A62024AF18E7D978');
    acKey.name = 'Diversified key for the AC generation';
    acKey.print();
    console.log(acKey.toString());
}
