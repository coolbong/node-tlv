...
function TLV (tag, value, encoding) {
    ...
    this.encoding = encoding || TLV.EMV;        //number
    this.tag    = TLV.adjustTag(tag, encoding); //string
    ...
    this.bLength = TLV.getBufferLength(this.value, encoding);
    //total length:  tag length , length length, value length 
    this.totalLength = this.bTag.length + this.bLength.length + this.bValue.length;

    if (this.encoding === TLV.DGI) {
        this.info = {};
        this.child = [];
...
TLV.parse = function(data, encoding) {
    ...
    if (encoding === TLV.DGI) {
        ...
        var tag = toHexString(buf.slice(0, 2));
        var lenTag = 2;
        var length;
...
TLV.adjustTag = function (tag, encoding) {
    ...
    if ((encoding === TLV.DGI) && (tag.length !== 4)) {
