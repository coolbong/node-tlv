	...
	function findAndReplace(tlv) {
		try {
			...
			if (tlv.tag === tagToUpdate) {
				const constructed = `${tlv.tag}${getLengthBasedOnEncoding(tlv.tag, valueToUpdate, encoding)}${valueToUpdate}`;
				return constructed;
			}
			...
			if (!constructedValue.length) return `${tlv.tag}${getLengthBasedOnEncoding(tlv.tag, tlv.value, encoding)}${tlv.value}`;
			// The values might have been changed, so constructing the value once again
			return `${tlv.tag}${getLengthBasedOnEncoding(tlv.tag, constructedValue, encoding)}${constructedValue}`;
		} catch (e) {
...
TLV.add = function (tlv, tagUnder, { tagToAdd, valueToAdd }, encoding = 0) {
	const constructedTLV = TLV.parse(tlv, encoding);
	const tagToUpdate = `${tagToAdd}${getLengthBasedOnEncoding(tagToAdd, valueToAdd, encoding)}${valueToAdd}`;
	const constructed = findAndAdd(constructedTLV);
	return constructed.toUpperCase();

	/**
	 * Constructs the TLV (Tag-Length-Value) value for a given tag and adds its value if found.
	 * @param {TLV} tlv
	 * @returns
	 */
	function findAndAdd(tlv) {
		try {
			...
			// Tag comparison
			if (tlv.tag === tagUnder) {
				const constructed = `${tlv.tag}${getLengthBasedOnEncoding(tlv.tag, _valueToAdd, encoding)}${_valueToAdd}`;
				return constructed;
			}
			let constructedValue = "";
			// Iterate all the childrens
...
TLV.delete = function (tlv, tagToDelete, encoding = 0) {
	const constructedTLV = TLV.parse(tlv, encoding);
	const constructed = findAndDelete(constructedTLV);
	return constructed.toUpperCase();
	 ...
	 * @returns
	 */
	function findAndDelete(tlv) {
		try {
			// Tag comparison
			if (tlv.tag === tagToDelete) {
				return "";
			}
			let constructedValue = "";
			// Iterate all the childrens
			for (let itr = 0; itr < tlv.child.length; itr++) {
				constructedValue += findAndDelete(tlv.child[itr]);
			}
			// If there is no children
			if (!constructedValue.length) return `${tlv.tag}${getLengthBasedOnEncoding(tlv.tag, tlv.value, encoding)}${tlv.value}`;
			// The values might have been changed, so constructing the value once again
 ...
 * @returns
 */
function getLengthBasedOnEncoding(tag, value, encoding) {
